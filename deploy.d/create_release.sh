...
do
    ...
    echo "*** Release service: '$LOCAL_PATH' -> '$RELEASE_REMOTE_PATH' on server: '$server' ***"
    ssh $server "mkdir -p $RELEASE_REMOTE_PATH $SHARED_REMOTE_PATH $SHARED_LOGS_REMOTE_PATH; ln -sf $SHARED_REMOTE_PATH/* $RELEASE_REMOTE_PATH/"
